...
def run(input_csv: str,
    ...
    # Format summary rows' item cells to 3-decimal strings so they look rounded in CSV
    def _format_row_3dp(row_series, cols):
        import math
        for _c in cols:
            v = row_series.get(_c, None)
            try:
                if v is not None and v == v:  # not NaN
                    row_series[_c] = f"{float(v):.3f}"
            except Exception:
                pass
        return row_series

    diff_row = _format_row_3dp(diff_row, item_cols)
    pb_row  = _format_row_3dp(pb_row, item_cols)

    df_out = pd.concat([df, pd.DataFrame([diff_row, pb_row])], ignore_index=True)
    df_out.to_csv(output_csv, index=False, float_format="%.3f", encoding="utf-8-sig")

    # Exam-level reliability
    k = len(item_cols)
    mean_total = float(total.mean())
    var_total = float(total.var(ddof=1))
    sd_total = float(np.sqrt(var_total)) if var_total >= 0 else float("nan")
    avg_difficulty = float(difficulty.mean())
    kr20_val = kr20(items_num, total)
    kr21_val = kr21(items_num, total)
    ...
    if item_report_csv:
        ...
        pd.DataFrame(report_df).to_csv(item_report_csv, index=False, float_format="%.3f", encoding="utf-8-sig")
