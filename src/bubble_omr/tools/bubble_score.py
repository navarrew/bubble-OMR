...
from ..config_io import load_config_any  # <-- YAML/JSON universal loader
import numpy as np
import cv2
...
class GridSpec:
    """
    A rectangular grid of bubbles.

    zone: [x1, y1, x2, y2] in normalized (0..1) page coordinates, or absolute pixels
    rows: number of rows in the grid (e.g., questions or symbols rows)
    cols: number of columns in the grid (e.g., choices per question or symbols columns)
    pad:  optional extra spacing factor used by your layout code
    shape: 'circle' (default) or others if you support them
    symbols: optional explicit symbol list (e.g., ['A','B','C','D','E'] or digits)
    positions: optional number used by some layouts (kept for backward compat)
    symbols_axis: 'rows' or 'cols' (how to traverse symbols)
    """
    ...
    symbols_axis: str = "rows"

    @staticmethod
    def from_dict(d: Dict[str, Any]) -> "GridSpec":
        """
        Build a GridSpec from a dict. Provides clear errors if required fields are missing.
        Accepts zone as list/tuple; coerces to 4-tuple of floats.
        """
        if d is None:
            raise ValueError("GridSpec.from_dict: input is None")

        # Required keys
        if "zone" not in d:
            raise ValueError("GridSpec: missing required key 'zone' (expected [x1,y1,x2,y2])")
        if "rows" not in d:
            raise ValueError("GridSpec: missing required key 'rows'")
        if "cols" not in d:
            raise ValueError("GridSpec: missing required key 'cols'")

        zone_raw: Sequence[float] = d["zone"]
        if not isinstance(zone_raw, (list, tuple)) or len(zone_raw) != 4:
            raise ValueError("GridSpec.zone must be a 4-length list/tuple [x1,y1,x2,y2]")

        zone = tuple(float(v) for v in zone_raw)  # type: ignore[assignment]

        return GridSpec(
            zone=zone,
            rows=int(d["rows"]),
            cols=int(d["cols"]),
            pad=float(d.get("pad", 0.0)),
            shape=str(d.get("shape", "circle")),
            symbols=list(d["symbols"]) if d.get("symbols") is not None else None,
            positions=int(d["positions"]) if d.get("positions") is not None else None,
            symbols_axis=str(d.get("symbols_axis", "rows")),
        )
...
class Config:
    """
    Top-level configuration. For backward compatibility:
    - You may provide either `answer_layouts` (list of GridSpec) or a single `answer_layout`.
      If both are provided, `answer_layouts` wins and `answer_layout` is set to the first.
    """
    ...
    def from_dict(raw: Dict[str, Any]) -> "Config":
        """
        Normalize dict loaded from YAML/JSON into a Config instance.
        """
        if not isinstance(raw, dict):
            raise ValueError("Config.from_dict: raw must be a dict")

        # Accept either answer_layouts (preferred) or answer_layout (fallback)
        al_list_raw = raw.get("answer_layouts")
        al_single_raw = raw.get("answer_layout")

        answer_layouts: Optional[List[GridSpec]] = None
        answer_layout: Optional[GridSpec] = None

        if isinstance(al_list_raw, list) and len(al_list_raw) > 0:
            answer_layouts = [GridSpec.from_dict(x) for x in al_list_raw]
            answer_layout = answer_layouts[0]
        elif al_single_raw is not None:
            answer_layout = GridSpec.from_dict(al_single_raw)
            answer_layouts = None
        else:
            raise ValueError(
                "Config must include either 'answer_layouts' (list) or 'answer_layout' (single)."
            )

        def _gs(key: str) -> Optional[GridSpec]:
            v = raw.get(key)
            return GridSpec.from_dict(v) if isinstance(v, dict) else None

        return Config(
            answer_layout=answer_layout,                    # type: ignore[arg-type]
            answer_layouts=answer_layouts,
            last_name_layout=_gs("last_name_layout"),
            first_name_layout=_gs("first_name_layout"),
            name_layout=_gs("name_layout"),
            id_layout=_gs("id_layout"),
            version_layout=_gs("version_layout"),
        )

# at top of tools/bubble_score.py add (or keep) these:
...
def load_config(path: str) -> Config:
    """
    Load a bubble-OMR config from YAML or JSON and normalize to Config.
    Accepts .yaml/.yml/.json (or no/unknown extension: tries YAML first, then JSON).
    Preserves legacy fields (positions/alphabet/digits, questions/choices/questions_per_row).
    """
    import math

    # Universal parse → dict (YAML preferred, then JSON)
    J = load_config_any(path)
    ...
    if "answer_layouts" in J and J["answer_layouts"]:
        ...
        # ✅ set primary layout to first entry
        if answer_layouts:
            answer_layout = answer_layouts[0]
    ...
    def gs(name: str) -> Optional[GridSpec]:
        if name not in J or J[name] is None:
            return None
